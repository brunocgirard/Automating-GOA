...
def show_welcome_page():
    ...
    The application has been connected to GitHub for version control.
    """)
...
# --- Quote Processing Page (Minimal Version) ---
def show_quote_processing():
    ...
    if uploaded_pdf:
                ...
                try:
                    ...
                    with st.status("Processing PDF...", expanded=True) as status:
                        ...
                        if not extracted_items:
                            ...
                            st.write(f"Extracted {len(extracted_items)} items from PDF.")
                        
                        # Store in session state
                        st.session_state.selected_pdf_items_structured = extracted_items
                        st.session_state.full_pdf_text = full_text
                        
                        # Use filename as quote_ref
                        quote_ref = uploaded_pdf.name.split('.')[0]
                        
                        # Basic client data payload
                        client_info_payload = {
                            "quote_ref": quote_ref,
                            "customer_name": "",
                            "machine_model": "",
                        ...
                        if save_client_info(client_info_payload):
                            ...
                            if save_priced_items(quote_ref, extracted_items):
                                ...
                                st.warning(f"Failed to save items to database.")
                            
                            # Save document content
                            if save_document_content(quote_ref, full_text, uploaded_pdf.name):
                                st.write(f"Document content saved for future reference.")
                            else:
                                st.warning(f"Failed to save document content.")
                        else:
                            st.error(f"Failed to create client record.")
                        
                        status.update(label="Processing Complete!", state="complete")
                    
                    # Display extracted data
                    if 'selected_pdf_items_structured' in st.session_state and st.session_state.selected_pdf_items_structured:
                        with st.expander("View Extracted Items", expanded=True):
                            df = pd.DataFrame(st.session_state.selected_pdf_items_structured)
        ...
        with col2:
            if st.button("View in CRM"):
                st.session_state.current_page = "CRM Management"
                st.rerun()

# --- Export Documents Page (Minimal Version) ---
def show_export_documents():
    st.title("📄 Export Documents")
    st.info("Export document functionality is being restored. Please check back soon.")

# --- CRM Management Page (Minimal Version) ---
def show_crm_management():
    st.title("📒 CRM Management")
    
    if st.button("Refresh CRM List", key=f"refresh_crm_btn_{st.session_state.run_key}"):
        load_crm_data()
    ...
    if st.session_state.all_crm_clients:
        ...
        display_cols = ['id', 'quote_ref', 'customer_name', 'machine_model', 'country_destination']
        display_df = df_all_clients[[c for c in display_cols if c in df_all_clients.columns]]
        
        st.dataframe(display_df, use_container_width=True, hide_index=True)
        
        # Client selection for detail view
        client_options = ["Select a Client..."] + [
            f"{c.get('customer_name', 'N/A')} - {c.get('quote_ref', 'N/A')} (ID: {c.get('id')})" 
            for c in st.session_state.all_crm_clients
        ]
        
        selected_client = st.selectbox("Select Client to View Details:", client_options, index=0)
        
        if selected_client != "Select a Client...":
            try:
                selected_id = int(selected_client.split("(ID: ")[-1][:-1])
                client_data = get_client_by_id(selected_id)
                
                if client_data:
                    # Display client details
                    st.subheader(f"Client Details: {client_data.get('customer_name', 'N/A')}")
                    
                    col1, col2 = st.columns(2)
                    with col1:
                        st.markdown(f"**Quote Reference:** {client_data.get('quote_ref', 'N/A')}")
                        st.markdown(f"**Customer Name:** {client_data.get('customer_name', 'N/A')}")
                        st.markdown(f"**Machine Model:** {client_data.get('machine_model', 'N/A')}")
                        st.markdown(f"**Country Destination:** {client_data.get('country_destination', 'N/A')}")
                    
                    with col2:
                        st.markdown(f"**Telephone:** {client_data.get('telephone', 'N/A')}")
                        st.markdown(f"**Contact Person:** {client_data.get('customer_contact_person', 'N/A')}")
                        st.markdown(f"**Customer PO:** {client_data.get('customer_po', 'N/A')}")
                    
                    # Display sold to and ship to addresses
                    with st.expander("Addresses", expanded=False):
                        st.markdown(f"**Sold To Address:**\n{client_data.get('sold_to_address', 'N/A')}")
                        st.markdown(f"**Ship To Address:**\n{client_data.get('ship_to_address', 'N/A')}")
                    
                    # Display priced items
                    ...
                    if priced_items:
                        ...
                        # Display columns
                        item_cols = ['description', 'quantity_text', 'selection_text']
                        items_display_df = items_df[[c for c in item_cols if c in items_df.columns]]
                        st.dataframe(items_display_df, use_container_width=True, hide_index=True)
                    else:
        ...
        st.info("No clients found in the database. Process a quote first.")
        
    # Add new client manually
    with st.expander("Add New Client Manually", expanded=False):
        with st.form("add_client_form"):
            new_quote_ref = st.text_input("Quote Reference (Required)")
            new_customer_name = st.text_input("Customer Name")
            new_machine_model = st.text_input("Machine Model")
            new_country = st.text_input("Country Destination")
            ...
            if submitted:
                if not new_quote_ref:
                    ...
                    new_client = {
                        ...
                        "country_destination": new_country,
                        "sold_to_address": "",
                        "ship_to_address": "",
                        "telephone": "",
                        "customer_contact_person": "",
                    ...
                    if save_client_info(new_client):
                        st.success(f"Client '{new_customer_name}' added successfully.")
                        load_crm_data()  # Refresh client list
                        st.rerun()
                    else:
