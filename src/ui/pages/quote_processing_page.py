...
This module contains the complete quote processing workflow with 4 main steps:
...
4. Process Machine - Select a machine and run LLM extraction and generate document
        ...
        if machines:
            ...
            with col1:
                if st.button("⬅️ Back (GOA Common Options)", key="goa_back_common"):
                    ...
                    st.rerun()
            with col2:
                if st.button("Extract Data and Generate Document →", type="primary", key=f"extract_machine_btn_{st.session_state.run_key}"):
                    with st.spinner(f"Extracting data for {selected_machine.get('machine_name')}..."):
                        ...
                        if success:
                            ...
                            with st.spinner("Generating document and saving to database..."):
                                success_gen = generate_and_save_document()
                                if success_gen:
